...
router.get("/:track_id", (req, res) => {
    ...
    pool.query(
        "SELECT album_id, album_title, artist_id, artist_name, tags, track_date_created, track_date_recorded, track_duration, track_genres, track_number, track_title, track_id FROM tracks WHERE track_id = $1",
        [cleanTrackId],
        (err, resp) => {
            if (err) {
                throw err;
...
router.get("/", (req, res) => {
    ...
    // updated
        ...
        pool.query(sqlStr, (err, resp) => {
            ...
            // 0 album ids, resp.rows.length == 0
            if (resp.rows.length > 0) {
    ...
    // updated
        ...
        let sqlStr =
            "SELECT DISTINCT artist_id FROM artists WHERE UPPER(artist_handle) LIKE UPPER('%" +
